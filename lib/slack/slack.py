...
class Slack:
    ...
    def send(self, text, blocks=None, attachments=None, thread_ts=None):
        self.client.chat_postMessage(channel=self.event_channel, text=text, blocks=blocks, attachments=attachments, thread_ts=thread_ts)

    def reply(self, text, blocks=None, attachments=None, thread_ts=None):
        text = f'<@{self.event_user}> ' + text
        self.send(text, blocks=blocks, attachments=attachments, thread_ts=thread_ts)
